...
import {
	DEFAULT_SETTINGS,
	ExtendedTaskListsSettingTab,
	ExtendedTaskListsSettings,
} from "src/settings";
...
export default class ExtendedTaskListsPlugin extends Plugin {
	...
	async onload() {
		...
		this.app.workspace.onLayoutReady(() => {
			this.registerEvent(
				this.app.vault.on("create", this.updateTodoFile),
			);
		});
		...
		this.registerEvent(
			this.app.vault.on("modify", async (file) => {
				if (
					file.name !== this.settings.todoFilename ||
	...
	getOrCreateTodoFile = async (vault: Vault): Promise<TFile> => {
		...
		try {
			...
			if (todoFileOrNull == null) {
				...
				throw new Error(
					`The retrieved TODO file is a folder: ${this.settings.todoFilename}`,
				);
			}
